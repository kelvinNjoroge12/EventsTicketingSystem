...
# Bootstrap Django
sys.path.append(r"c:\Users\lmbua\Downloads\EventFrontend\strathmore_university_backend")
os.environ.setdefault("DJANGO_SETTINGS_MODULE", "config.settings.development")
django.setup()

from apps.events.models import Event

BASE_MEDIA = r"c:\Users\lmbua\Downloads\EventFrontend\strathmore_university_backend\media\event_covers"
    ...
    for title, filename in MAPPING.items():
        ...
        if event:
            ...
            if os.path.exists(filepath):
                ...
                print(f"âœ… Updated image for: {title}")
            else:
                print(f"âŒ File not found: {filepath}")
        else:
            print(f"â“ Event not found: {title}")
