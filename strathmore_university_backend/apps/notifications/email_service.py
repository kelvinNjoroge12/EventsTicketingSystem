...
class EmailService:
    def __init__(self):
        ...
        self.from_email = getattr(settings, 'DEFAULT_FROM_EMAIL', 'noreply@strathmoreuniversity.com')

    def send_verification_email(self, user):
        token = generate_secure_token(user, email_verification_token_generator)
        verification_url = f"{self.frontend_url}/verify-email?token={token}"
        
        subject = "Verify your Strathmore University email"
    ...
    def send_password_reset(self, user):
        ...
        subject = "Reset your Strathmore University password"
        message = f"Hi {user.first_name},\n\nYou requested a password reset. Click the link below to set a new password:\n\n{reset_url}\n\nIf you didn't request this, you can safely ignore this email.\n\nThanks!\nStrathmore University Team"
        
        send_mail(subject, message, self.from_email, [user.email], fail_silently=True)

    def send_organizer_welcome(self, user):
        subject = "Welcome to Strathmore University Organizer!"
