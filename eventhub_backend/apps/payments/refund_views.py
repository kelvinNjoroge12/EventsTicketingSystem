...
class RefundOrderView(APIView):
    ...
    def post(self, request):
        ...
        # Authorization: organizer of the event, admin/staff, or the attendee themselves
        user = request.user
        is_organizer = order.event and order.event.organizer_id == user.id
        is_attendee = order.attendee_id == user.id
        if not (user.is_staff or user.role == "admin" or is_organizer or is_attendee):
            return Response(
                {"detail": "You do not have permission to refund this order."},
