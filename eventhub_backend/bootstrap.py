"""
bootstrap.py — Run from eventhub_backend/ folder with:
   python bootstrap.py
...
os.environ.setdefault("SECRET_KEY", "eventhub-dev-secret-key-2025")
...
# ── Create/ensure organizer user ─────────────────────────────
print("\n[1/7] Setting up organizer account...")
if User.objects.filter(email="admin@eventhub.com").exists():
    organizer = User.objects.get(email="admin@eventhub.com")
    print("  Already exists — using admin@eventhub.com")
else:
    organizer = User.objects.create_superuser(
        email="admin@eventhub.com", password="Admin1234!",
        first_name="EventHub", last_name="Admin",
    )
    print("  Created: admin@eventhub.com / Admin1234!")
...
OrganizerProfile.objects.get_or_create(
    user=organizer,
    defaults={"organization_name": "EventHub Official", "is_approved": True, "is_verified": True},
)
print("  Organizer profile ready ✓")
...
EVENTS = [
    ...
    },
    {
        ...
        "speakers": [
            {"name": "DJ Afroking", "title": "Headline DJ & Producer", "org": "Independent", "bio": "4× African Music Award winner blending traditional rhythms with electronic production.", "mc": False, "seed": 512},
    ...
    },
    {
        ...
        "description": "Full-day intensive training for marketers, entrepreneurs and content creators. Master paid media, SEO, social strategy and analytics — led by practitioners who managed $50M+ in ad spend.\n\nLeave with a 90-day roadmap, private community access and 6-month Q&A support. Laptops required.",
        "category": "Education", "format": "in_person",
...
for idx, ev in enumerate(EVENTS, 1):
    ...
    with transaction.atomic():
        ...
        print(f"     ✅ Created — {len(ev['tickets'])} tickets | {len(ev['speakers'])} speakers | {len(ev['schedule'])} sessions | {len(ev['sponsors'])} sponsors")
